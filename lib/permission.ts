...
import { navMain, navAdmin } from "@/components/constants/menu";
...
type UrlEnumType = RemoveSlash<MenuUrlEnum<MainMenuItemType> | MenuUrlEnum<AdminMenuItemType>>;
...
export type Subject = PrettyType<EnumLikeType<typeof subjects> | FilterNonEmtArr<typeof subjects>>;
...
export const hasPermission = (role: Role, subject: Subject, action: Action) => !!roles[role]?.[subject]?.[action];

export const hasPagePermission = (role: Role, subject: Subject) => !!Object.values(roles[role]?.[subject] ?? {}).some(c => !!c);