...
export const authConfig = {
  ...
  callbacks: {
    authorized({ auth, request: { nextUrl } }) {
      ...
      const isSomewhere = [...menuData.navMain, ...menuData.navAdmin, ({url: '/'} as SubMenuItemType)].some((c) => c.subRoutes ? pathname.startsWith(c.url) : c.url === pathname);

      if (isLoggedIn && isAuthRoute) return Response.redirect(new URL("/", nextUrl));

      if (!isLoggedIn && isSomewhere) return Response.redirect(new URL("/login", nextUrl));
