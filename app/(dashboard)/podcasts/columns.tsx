...
import { PodcastItemType } from "./schema";
...
const Action = ({ row }: CellContext<PodcastItemType, unknown>) => {
  ...
  return (
    <div className="flex gap-4 justify-end me-2">
      ...
      <DeleteDialog
        ref={deleteDialogRef}
        loading={loading}
        permissionSubject="podcasts"
        action={() => {
          setLoading(true);
          deletePodcast(row.original.id)
...
export const podcastColumns: ColumnDef<PodcastItemType>[] = [
  {
    accessorKey: "id",
    header: "ID",
    cell: ({ row }) => {
