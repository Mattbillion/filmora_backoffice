...
import { MoodItemBodyType, MoodItemItemType, moodItemSchema } from "../schema";
import { patchMoodItem } from "../actions";
...
export function UpdateDialog({
  ...
  const form = useForm<MoodItemBodyType>({
    ...
    defaultValues: {
      ...
      status: initialData.status.toString() as MoodItemBodyType["status"],
      banner: initialData.banner,
      body: initialData.body,
      audio: initialData.audio,
      audioDuration: initialData.audio_duration ?? 0,
      moodListId: initialData.mood_list_id,
  ...
  function onSubmit(values: MoodItemBodyType) {
    startTransition(() => {
      patchMoodItem({ ...values, id: initialData.id })
        .then(() => {
  ...
  if (!hasPermission(role, "moods.item", "update")) return null;
  return (
    <FormDialog
      ref={dialogRef}
      form={form}
