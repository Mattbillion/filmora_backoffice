...
import { getCategoryAttributesList } from './actions';
...
export default async function CategoryAttributesPage(props: {
  ...
  const { data } = await getCategoryAttributesList(searchParams);

  return (
    <>
      <div className="flex items-start justify-between">
        <Heading
          title={`CategoryAttributes list (${data?.pagination?.total ?? data?.data?.length})`}
        />
        <CreateDialog>
