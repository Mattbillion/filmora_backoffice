...
import { BookBodyType, BookItemType, bookSchema } from "../schema";
import { patchBook } from "../actions";
...
export function UpdateDialog({
  ...
  const form = useForm<BookBodyType>({
    ...
    defaultValues: {
      ...
      status: initialData.status.toString() as BookBodyType["status"],
  ...
  function onSubmit(values: BookBodyType) {
    startTransition(() => {
      patchBook({ ...values, id: initialData.id })
        .then(() => {
  ...
  if (!hasPermission(role, "albums", "create")) return null;
  return (
    <FormDialog
      ref={dialogRef}
      form={form}
